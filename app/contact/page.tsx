...
export default function ContactPage() {
  ...
  // 🌟 UPDATE: Added 'city' in state
  const [waData, setWaData] = useState({ name: "", email: "", city: "", role: "Customer" });

  // Main Form Submit
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Yahan aap apna backend API ya Email service (jaise Resend/Nodemailer) connect kar sakte hain
    setSubmitted(true);
    setTimeout(() => {
      setSubmitted(false);
  ...
  const handleWaSubmit = (e: React.FormEvent) => {
    ...
    // 🌟 UPDATE: Formatting the message for WhatsApp including City
            ...
            <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button 
                    type="submit" 
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-xl shadow-md transition-all active:scale-[0.98]"
                  >
                    Send Message 🚀
                  </button>
                </form>
