...
export default async function TouristPlacePage({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    // 🌟 SMART NEAREST PLACES PARSER
    let nearestList: any[] = [];
    let oldNearestText = "";
    
    let rawNearest = meta.nearestPlaces;

    // Agar database mein array ki jagah galti se stringify ho gaya ho
    if (typeof rawNearest === 'string' && rawNearest.trim() !== '') {
      try {
        const parsed = JSON.parse(rawNearest);
        if (Array.isArray(parsed)) {
          rawNearest = parsed; // Successful parse
        } else {
          oldNearestText = rawNearest; // Normal string
        }
      } catch (e) {
        oldNearestText = rawNearest; // Agar parse fail ho jaye, toh regular text maan lo
      }
    }

    // Ab agar properly array mil gaya hai, tab database se fetch karein
    if (Array.isArray(rawNearest) && rawNearest.length > 0) {
      ...
      nearestList = Array.from(new Map(combined.map(item => [item.id, item])).values());
    }
      ...
      <main className="min-h-screen bg-slate-50 pb-20 font-sans selection:bg-blue-600 selection:text-white">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-4">
                {placeCats.map((cat: string, index: number) => (
                  <Link 
                    ...
                    className="bg-amber-500 hover:bg-amber-400 text-slate-900 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest shadow-md transition-colors cursor-pointer"
                  >
                    {cat}
                  </Link>
                ))}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-12 py-8">
          ...
          {(meta.timing || meta.entryFee || meta.bestTimeToVisit || meta.howToReach) && (
            <section className="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 mb-10 w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {meta.howToReach && (
                  <div className="flex gap-4 items-start bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-sm sm:col-span-2 lg:col-span-3">
          ...
          <div className="w-full space-y-10">
            ...
            {/* 🌟 CLICKABLE NEAREST PLACES SECTION */}
            {(nearestList.length > 0 || oldNearestText) && (
              <section className="bg-emerald-50 text-emerald-950 p-8 md:p-10 rounded-[2.5rem] border border-emerald-100 shadow-sm">
                <h2 className="text-2xl font-black mb-6 flex items-center gap-2"><span>📍</span> Nearby Places & Attractions</h2>
                
                {nearestList.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {nearestList.map((item) => {
                      const nearMeta = typeof item.metadata === 'object' && item.metadata !== null ? item.metadata : {};
                      const nearImg = item.image || nearMeta.image || '/ITO LOGO.png';
                      return (
                        <Link key={item.id} href={`/places/${item.slug || item.id}`} className="bg-white border border-emerald-200 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col">
                          <div className="h-40 w-full overflow-hidden bg-emerald-100 relative">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={nearImg} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                          </div>
                          <div className="p-5 flex-1 flex flex-col justify-between">
                            ...
                            <span className="text-xs font-bold text-emerald-700 mt-3 inline-block bg-emerald-100 w-max px-3 py-1.5 rounded-full group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                              Explore Place →
                            </span>
                          </div>
                        </Link>
