...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    ...
    const thumbnail = safeGallery.length > 0 ? safeGallery[0] : `${siteUrl}/default-cab.jpg`
...
export default async function CabDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    // 🌟 FIX 4: Safe Objects and Arrays checks
    const meta = typeof cab.metadata === 'object' && cab.metadata !== null ? cab.metadata : {}
    
    // Extract Origins and Destinations FIRST to filter Tourist Places securely
    const safeCabLocation = typeof cab.location === 'string' ? cab.location : (Array.isArray(cab.location) ? cab.location.join(' > ') : 'India')
    const rawOrigin = String(meta.pickupCity || meta.pickupPoint || meta.serviceCity || safeCabLocation)
    const rawDrop = String(meta.dropCity || meta.dropPoint || safeCabLocation)
    
    const aiOrigin = rawOrigin.replace(/ > /g, ', ').split(' to ')[0].trim() || 'India'
    const aiDrop = rawDrop.replace(/ > /g, ', ').split(' to ').pop()?.trim() || ''
    
    // Clean Target City for filtering Places
    const targetCity = (aiDrop !== 'Destination' && aiDrop !== 'India') ? aiDrop : (safeCabLocation.split(' > ').pop()?.trim() || '')
    ...
    // Find minimum price for schema safely
    const cabPrices = typeof meta.cabPrices === 'object' && meta.cabPrices !== null ? meta.cabPrices : {}
    ...
    const defaultFaqs = [
      ...
      {
        question: `What types of cabs are available from ${aiOrigin} to ${aiDrop || targetCity || 'your destination'}?`,
    ...
    const rawFaqs = Array.isArray(meta.faqs) ? meta.faqs : []
    const validVendorFaqs = rawFaqs.filter((f: any) => f && f.question && f.question.trim() !== '' && f.answer && f.answer.trim() !== '')
      ...
      <main className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-800">
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 mt-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-emerald-50 p-6 md:p-8 rounded-3xl border border-emerald-100 shadow-sm">
                    ...
                    <ul className="text-emerald-900 font-medium space-y-3">
                      ...
                      {Array.isArray(meta.customInclusions) && meta.customInclusions.length > 0 && meta.customInclusions.map((item: string, idx: number) => (
                  ...
                  <div className="bg-rose-50 p-6 md:p-8 rounded-3xl border border-rose-100 shadow-sm">
                    ...
                    <ul className="text-rose-900 font-medium space-y-3">
                      ...
                      {Array.isArray(meta.customExclusions) && meta.customExclusions.length > 0 && meta.customExclusions.map((item: string, idx: number) => (
            ...
            {/* BENEFITS, SAFETY, FLEET & TESTIMONIALS */}
            <section className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100 flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-800 text-sm mt-1">No hidden charges or surprise tolls. What you see is what you pay for your {aiOrigin} to {aiDrop || 'destination'} taxi service.</p>
                  </div>
                </div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200">
              {meta.howToReach && typeof meta.howToReach === 'string' && (
            ...
            <section className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                  <div className="text-yellow-400 text-xl mb-3">★★★★★</div>
                  <p className="text-slate-700 italic mb-4">"Best taxi service for {aiDrop || 'this route'}. I compared prices with other apps, but this was more transparent with no hidden night charges."</p>
        ...
        {/* RELATED CAB SECTIONS */}
        <RelatedCabSections 
          cabId={cab.id} 
          vendorId={cab.vendor_id} 
          location={typeof cab.location === 'string' ? cab.location : String(cab.location)} 
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-red-50 p-6">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-3xl w-full border-2 border-red-200">
          <h1 className="text-2xl font-black text-red-600 mb-4">⚠️ Cab Page Crash Report</h1>
