...
export default function AddBlogPage() {
  ...
  useEffect(() => {
    ...
    if (savedCats) {
      ...
      if (parsedCats.length > 0) setCategory(parsedCats[0])
    } else {
      const defaultCats = ["Travel Guide", "Tips & Tricks", "Itinerary", "Food & Culture"]
  ...
  // ReactQuill Toolbar options (includes lists, links, images)
  const quillModules = {
    toolbar: [
      [{ 'header': [2, 3, 4, false] }],
      ['bold', 'italic', 'underline', 'strike', 'blockquote'],
      [{ 'list': 'ordered' }, { 'list': 'bullet' }],
      ['link', 'image'], // Allows adding links and image URLs
      ['clean']
    ]
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .insert([{
          ...
          category: 'blog', // Core system category remains 'blog'
          location: location,
          price: 0,
          status: 'approved',
          metadata: metadata
    ...
    if (error) {
      ...
      setMessage({ type: 'success', text: 'Blog article successfully published!' })
      setSubmitting(false)
    ...
    <div className="min-h-screen p-4 md:p-8 bg-gray-50">
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="bg-indigo-600 p-6 text-white flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-extrabold">Add New Blog Article</h1>
        ...
        <div className="p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <button type="submit" disabled={submitting} className="w-full bg-indigo-600 text-white font-bold py-4 rounded-xl hover:bg-indigo-700 text-lg shadow-lg transition-transform transform hover:scale-[1.01]">
              {submitting ? 'Publishing Blog...' : 'Publish Blog Article'}
            </button>
          </form>
        </div>
      </div>
