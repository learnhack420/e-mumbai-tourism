...
export default function EditTouristPlace({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    // 2. Data Update karna (Edit function)
    const { error } = await supabase
      ...
      .update({
        ...
        location: formData.cityName ? `${formData.cityName}, ${formData.stateName}` : formData.stateName,
        metadata: metadata
      })
      .eq("id", resolvedParams.id) // Edit logic using ID

    if (error) {
      setMessage({ type: "error", text: "Error updating: " + error.message })
      setSubmitting(false)
    } else {
      setMessage({ type: "success", text: "✅ Tourist Attraction successfully updated!" })
      setSubmitting(false)
      setTimeout(() => { router.push("/admin") }, 2000)
    }
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center font-bold text-xl text-amber-600">Loading place details...</div>
    ...
    <div className="min-h-screen p-4 md:p-8 bg-gray-50 font-sans">
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="bg-amber-600 p-6 text-white flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-black">✏️ Edit Tourist Attraction</h1>
            <p className="text-amber-100 text-sm mt-1">Update destination details, descriptions & SEO metadata</p>
          </div>
          <Link href="/admin" className="bg-amber-700 hover:bg-amber-800 px-4 py-2 rounded-lg font-medium text-sm transition-colors">
            &larr; Back to Admin
          </Link>
        </div>
        ...
        <div className="p-6 md:p-8">
          {message.text && (
            <div className={`mb-6 p-4 rounded-xl text-sm font-bold ${message.type === "error" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>
              {message.text}
            </div>
          )}
          ...
          <form onSubmit={handleUpdate} className="space-y-8">
            ...
            <div className="border border-gray-200 p-6 rounded-xl space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <input type="text" list="synced-cities" required className="w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 font-semibold" value={formData.cityName} onChange={(e) => setFormData({...formData, cityName: e.target.value})} placeholder="e.g. Mumbai" />
                  <datalist id="synced-cities">
                    {savedCities.map((city, idx) => (<option key={idx} value={city} />))}
                  </datalist>
                </div>
              </div>
            </div>
            ...
            <div className="border border-gray-200 p-6 rounded-xl space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <input type="text" className="w-full px-4 py-3 border rounded-xl outline-none" value={formData.entryFee} onChange={(e) => setFormData({...formData, entryFee: e.target.value})} placeholder="Free or ₹50" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Timing</label>
                  <input type="text" className="w-full px-4 py-3 border rounded-xl outline-none" value={formData.timing} onChange={(e) => setFormData({...formData, timing: e.target.value})} placeholder="9 AM - 6 PM" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Best Time To Visit</label>
                  <input type="text" className="w-full px-4 py-3 border rounded-xl outline-none" value={formData.bestTime} onChange={(e) => setFormData({...formData, bestTime: e.target.value})} placeholder="October to March" />
                </div>
              </div>
            </div>
            ...
            <div className="pt-6 border-t">
              <button type="submit" disabled={submitting || isUploading} className="w-full bg-amber-600 hover:bg-amber-700 text-white py-4 rounded-2xl font-black text-lg shadow-lg transition-transform transform hover:scale-[1.01]">
                {submitting ? "Updating Attraction..." : "Update Attraction Page"}
              </button>
            </div>
